...
void cmd_renombrar(char **args){
    if (args[1] == NULL || args[2] == NULL) {
        printf("Uso: renombrar <archivo_viejo> <archivo_nuevo>\n");
        return;
    }

    // rename(): Renombra un archivo o directorio. Retorna 0 en éxito, -1 en error.
    if (rename(args[1], args[2]) == 0) {
        printf("Archivo '%s' renombrado a '%s' correctamente.\n", args[1], args[2]);
    } else {
